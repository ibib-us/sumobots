...
It Begins in Brockport (IBiB) is developing its own sumobot league to introduce the community to competitive robotics.

Sumobots are robots that compete to push one another out of a ring. Ours are autonomous, which means they need to figure out how to do this all on their own. Ours are also built using inexpensive and easy-to-acuire materials, which means you can try out autonomous competitive robotics without having to invest a lot of cash.

![Sumo Louis](./img/sumobot-building.jpg)

Here, you will find the hardware, the software and the rules for sumobots.

# Hardware

To be written. The files you need to 3D print the body of your sumobot are in the [stl](/stl/) directory.

# Software

The software can be found in the [code](/code/) directory. There are four files that are needed for your sumobot to run:
...
Since this sumobot is running version 7, it requires the version 7 library. However, Adafruit no longer supports this version, and it is recommended we upgrade to the latest (which is 9 at the moment - this is the version you can find in this repository). Here are the steps:
